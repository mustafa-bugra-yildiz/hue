...
static void
printNode(struct node *n, int level)
{
	switch (n->t) {
	case N_FN:
...
static int
patternMatches(enum type *pattern)
{
	int		sLen, pLen, i;
...
static int
compile(struct node *n, int level)
{
	int		lhs, rhs;
...
GETOPT_DONE:
	...
	*rp++ = (struct rule){
		mkFn, {
			N_IDENT, N_EQ, N_EXPR, N_SEMI
		}
	};
	*rp++ = (struct rule){
		mkFn, {
			N_IDENT, N_EQ, N_ADD, N_SEMI
		}
	};
	*rp++ = (struct rule){
		mkFn, {
			N_IDENT, N_EQ, N_NUMBER, N_SEMI
		}
	};
